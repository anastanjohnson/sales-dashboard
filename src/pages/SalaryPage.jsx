...
export default function SalaryPage({ salaryData }) {
  const [selectedMonth, setSelectedMonth] = useState(salaryData.length - 1);
  const [department, setDepartment] = useState("All");
  const [view, setView] = useState("chart");
  const monthData = salaryData[selectedMonth];

  const visibleEmployees = useMemo(
    () => monthData.employees.filter((employee) => department === "All" || employee.department === department),
  ...
  const reset = () => {
    setSelectedMonth(salaryData.length - 1);
    setDepartment("All");
    setView("chart");
  };
  ...
  return (
    <div className="dashboard salary-page">
      <div className="dashboard__header">
        <div><h1>Salary</h1><p className="dashboard__subtitle">Employee salary data from the 2026 monthly worksheets.</p></div>
      ...
      <div className="panel salary-panel">
        ...
        {view === "chart" ? (
          <div className="salary-chart"><ResponsiveContainer width="100%" height={420}><BarChart data={visibleEmployees} margin={{ top: 16, right: 18, left: 4, bottom: 82 }}>
            <CartesianGrid vertical={false} stroke="var(--grid)" />
            <XAxis dataKey="name" interval={0} angle={-42} textAnchor="end" height={90} tickLine={false} axisLine={{ stroke: "var(--baseline)" }} tick={{ fill: "var(--text-muted)", fontSize: 11 }} />
            <YAxis tickFormatter={shortCurrency} tickLine={false} axisLine={false} tick={{ fill: "var(--text-muted)", fontSize: 12 }} width={52} />
            <Tooltip formatter={(value) => [currency.format(value), "Salary"]} contentStyle={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10 }} />
            <Bar dataKey="salary" name="Salary" radius={[4, 4, 0, 0]} maxBarSize={38}>{visibleEmployees.map((employee) => <Cell key={`${employee.department}-${employee.name}`} fill={employee.department === "Kitchen" ? "var(--series-1)" : "var(--series-2)"} />)}</Bar>
          </BarChart></ResponsiveContainer></div>
        ) : (
          <div className="table-wrap"><table className="salary-table"><thead><tr><th>Employee</th><th>Department</th><th>Salary</th></tr></thead><tbody>
            {visibleEmployees.map((employee) => <tr key={`${employee.department}-${employee.name}`}><td className="salary-table__month">{employee.name}</td><td>{employee.department}</td><td>{currency.format(employee.salary)}</td></tr>)}
          </tbody></table></div>
        )}
      </div>
    </div>
