...
export default function SalesPage() {
  ...
  const metricConfig = {
    revenue: { label: "Revenue", formatter: money.format, labelFormatter: compactMoney.format, short: compactMoney.format, color: "var(--series-1)" },
    tips: { label: "Tips", formatter: money.format, labelFormatter: compactMoney.format, short: compactMoney.format, color: "var(--series-2)" },
  }[metric];
  const comparisonData = monthOrder
    .map((month) => {
      const year2025 = salesData.find((row) => row.year === 2025 && row.month === month);
